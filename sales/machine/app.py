...
DATA_FILE = Path(__file__).parent / "data" / "machine.xlsx"
# =========================================================
# 2. 样式
# =========================================================
st.markdown(
...
def prepare_income_long(income_wide: pd.DataFrame):
    ...
    def extract_category(name):
        if "0-30%的机器" in name:
            return "0-30%的机器"
        elif "0-70%的机器" in name:
            return "0-70%的机器"
        elif "0-100%的机器" in name:
            return "0-100%的机器"
        else:
            return pd.NA

    df["类别"] = name_series.apply(extract_category)
...
def prepare_freq_long(freq_wide: pd.DataFrame):
    ...
    # 提取类别信息
    def extract_category(name):
        if "0-30%机器频次均值" in name:
            return "0-30%机器频次均值"
        elif "0-70%机器频次均值" in name:
            return "0-70%机器频次均值"
        elif "0-100%机器频次均值" in name:
            return "0-100%机器频次均值"
        else:
            return pd.NA
...
def make_grouped_horizontal_bar(
    ...
    fig.update_layout(
        ...
        paper_bgcolor="white",
        plot_bgcolor="white",
        margin=dict(t=10, b=10, l=0.1, r=30),
        legend=dict(
            orientation="h",
            x=0.00,
            y=1.02,
...
with c2:
    info_card("机器月收入均值(年度)", fmt_yuan(row.get("机器月收入均值"), 2))
with c3:
    info_card("机器月均频次(年度)", fmt_num(row.get("机器月均频次"), 2))
with c4:
    # 注意字段名更新
    info_card("Top30%机器月收入均值(年度)", fmt_yuan(row.get("top30%的机器月收入"), 2))
with c5:
    # 注意字段名更新
    info_card("TOP30%机器频次均值(年度)", fmt_num(row.get("TOP30%机器频次均值"), 2))
...
income_color_map = {
    "0-30%的机器": "#6395a5",  # 浅蓝色
    "0-70%的机器": "#00a8ed",  # 蓝色
    "0-100%的机器": "#051c2c"  # 深蓝色
}

# 更新频次分层类别和颜色映射
freq_category_order = [
    "0-30%机器频次均值",  # 产值最低的30%机器的频次均值
    "0-70%机器频次均值",  # 产值最低的70%机器的频次均值
    "0-100%机器频次均值"  # 所有机器的频次均值
]
freq_color_map = {
    "0-30%机器频次均值": "#6395a5",  # 浅蓝色
    "0-70%机器频次均值": "#00a8ed",  # 蓝色
    "0-100%机器频次均值": "#051c2c"  # 深蓝色
}

st.markdown('<div class="panel-card">', unsafe_allow_html=True)
st.markdown('<div class="chart-title">机器收入分层月度表现（按产值从低到高）</div>', unsafe_allow_html=True)
...
st.markdown('<div class="panel-card">', unsafe_allow_html=True)
st.markdown('<div class="chart-title">机器频次分层月度表现（按产值从低到高）</div>', unsafe_allow_html=True)
